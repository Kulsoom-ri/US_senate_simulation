...
\[ 
\text{Simulation Accuracy} = \beta_0 + \beta_1 \cdot \text{year} + \beta_2 \cdot \text{final\_vote\_result} + \beta_3 \cdot \text{previous\_action\_length} + \beta_4 \cdot \text{required\_majority} + \beta_5 \cdot \text{measure\_summary\_length} + \beta_6 \cdot \text{not\_voting} + \beta_7 \cdot \text{num\_cosponsors} + \beta_8 \cdot \text{vote\_margin} + \beta_9 \cdot \text{introduced\_party} + \beta_{10} \cdot \text{topic} + \beta_{11} \cdot \text{type\_vote} + \epsilon
\]
